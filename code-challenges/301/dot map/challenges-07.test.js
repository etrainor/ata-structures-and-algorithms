...
const forLoopTwoToThe = (arr) => {
  ...
  for(let i = 0 ; i < arr.length ; i++){
    let insideArray = arr[i];
    newArray.push(Math.pow(2, insideArray));
  }
  return newArray;
};
...
const mapTwoToThe = (arr) => {
  return arr.map(variable => Math.pow(2, variable));
}
...
const charCode = (arr) => {
  return arr.map(variable => variable.charCodeAt());
};
...
const extractStats = (arr) => {
  return arr.map(stuff =>{
    return {
      name: stuff.stat.name,
      total: stuff.effort + stuff.baseStat,
    }
  })
};
